...
fun LoginMainScreen(
    ...
    userInfoStateFlow: StateFlow<LoginUserInfoState>,
    ...
    when (val uiState = uiStateFlow.collectAsState().value) {
        ...
        is LoginUiState.Loaded -> {
            onInteraction(LoginInteraction.OnNavigateToMainAppGraph(uiState.profile))
        }
...
fun LoginInitialScreen(
    modifier: Modifier = Modifier,
    onInteraction: (LoginInteraction) -> Unit,
    uiStateFlow: StateFlow<LoginUserInfoState>,
    onNavigateSignUp: () -> Unit,
    isShowProgressBar: Boolean,
) {
    val uiState: LoginUserInfoState = uiStateFlow.collectAsState().value
    ...
    Box(
        ...
        Column(
            ...
            AuthBtnComponent(
                ...
                isProgressBarShow = isShowProgressBar
            )
            Row(
                modifier = Modifier
                    .fillMaxWidth()
...
fun LoginScreenPreview() {
    MaterialTheme {
        LoginInitialScreen(
            onInteraction = {},
            uiStateFlow = MutableStateFlow(LoginUserInfoState()),
            onNavigateSignUp = {},
            isShowProgressBar = false
        )
    }
